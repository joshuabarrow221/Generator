...
\brief   Reads a list of GENIE event files (GHEP format), `cherry-picks' events with a given 
         ...
         Synopsis:
           gevpickfromspectrum -i list_of_input_files -t topology  
                   [-o output_file]
         ...
         Examples:
           ...
           (1)  % gevpickfromspectrum -i "*.ghep.root" -t numu_nc_1pi0
...
\author  Joshua Barrow <jbarrow \at fnal.gov>
...
// func prototypes
// argc = input filenames; argv = topology string; argf = ROOT TFile with spectra information;
// args = ROOT TH1's stored in same ROOT TFile
void   GetCommandLineArgs             (int argc, char ** argv, char ** argf, char ** args); 
void   RunCherryPickerFromSpectrum    (void);
...
string      gOptSpecFileName;  ///< input ROOT TFile name with "correct" spectrum
string      gOptSpecHistNames; ///< input ROOT TH1 names inside TFile with "correct" spectrum
...
void RunCherryPickerFromSpectrum(void)
{
  ...
  LOG("gevpickfromspectrum", pFATAL) 
      << "Processing " << nfiles
      << (nfiles==1 ? " file " : " files ");
  ...
  while (( chEl=(TChainElement*)next_file() )) {
     ...
     if (!mcrec) {
       LOG("gevpickfromspectrum", pERROR) << "Null MC record";
       return;
     }
     Long64_t nmax = ghep_tree->GetEntries();
     LOG("gevpickfromspectrum", pNOTICE) 
        << "* Analyzing: " << nmax 
     ...
     for(Long64_t iev = 0; iev < nmax; iev++) {
       ...
       LOG("gevpickfromspectrum", pDEBUG) << rec_header;
       LOG("gevpickfromspectrum", pDEBUG) << event;
  ...
  LOG("gevpickfromspectrum", pFATAL) << "Done!";
}
//____________________________________________________________________________________
bool AcceptEvent(const EventRecord & event)
{
  ...
  //Added by J. L. Barrow
  double nuenergy  = event.Probe()->Energy();
...
void GetCommandLineArgs((int argc, char ** argv, char ** argf, char ** args)
{
  ...
  RunOpt::Instance()->ReadFromCommandLine(argc,argv,argf,args);

  // Parse run options for this app

  CmdLnArgParser parser(argc,argv,argf,args);
  ...
  if( parser.OptionExists('t') ) {
    ...
    if(gPickedTopology == kPtUndefined) {
      LOG("gevpickfromspectrum", pFATAL) << "Unknown topology (" << topo << ")";
      gAbortingInErr = true;
      exit(1);
    }

  } else {
    LOG("gevpickfromspectrum", pFATAL) << "Unspecified event topology";
    gAbortingInErr = true;
    exit(1);
  }

  // parse ROOT filenames with TH1s inside
  if( parser.OptionExists('f') ) {
    gOptOutFileName = parser.ArgAsString('f');
    if( parser.OptionExists('s') ) {
      gOptOutFileName = parser.ArgAsString('s');
    }
  } else {
    LOG("gevpickfromspectrum", pINFO)
      << "Unspecified input spectrum ROOT TFile name and TH1 name(s) - Ignoring";
  }
...
void PrintSyntax(void)
{
  ...
  string thisfile = basedir + string("/src/Apps/gEvPickFromSpectrum.cxx");
